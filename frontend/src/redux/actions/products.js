...
export const get_product = (productId) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `${import.meta.env.VITE_API_URL}/api/product/product/${productId}`,
      config
    );
...
export const get_related_products = (productId) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `${import.meta.env.VITE_API_URL}/api/product/related/${productId}`,
      config
    );
  ...
  (category_id, price_range, sort_by, order) => async (dispatch) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/product/by/search`,
        body,
        config
      );
